...
public class FormationController : MonoBehaviour {
    public int wave; //Contador de la oleada actual del juego
    ...
    void SpawnP (string p) {
        Transform freePosition = transform.Find(p);
        GameObject enemy = Instantiate(enemyPrefab, freePosition.position, Quaternion.identity) as GameObject;
        enemy.transform.parent = freePosition;
    }

    void SpawnM(string p)
    {
        Transform freePosition = transform.Find(p);
        GameObject enemy2 = Instantiate(enemyMedium, freePosition.position, Quaternion.identity) as GameObject;
